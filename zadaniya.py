...
def test_my_sum():
    ...
    test(12, zadaniya2.my_sum((10, 2, 0)))
    test(144, zadaniya2.my_sum((10, 20, 100, 14)))
    test(12, zadaniya2.my_sum((-50, 200, -100, -50, 12)))
...
def test_shortener():
    ...
    for i in range(len(sourceStrings)):
        test(testStrings[i], zadaniya2.shortener(sourceStrings[i]))

def test_compare_ends():
    print("We are testing compare_ends!" + '\n')
    test(zadaniya2.compare_ends(('aba', 'xyz', 'aa', 'x', 'bbb')), 3)
    test(zadaniya2.compare_ends(('', 'x', 'xy', 'xyx', 'xx')), 2)
    test(zadaniya2.compare_ends(('aaa', 'be', 'abc', 'hello')), 1)

def test_spam():
    print("We are testing spam!" + '\n')
    test(zadaniya2.spam((3)), 'bulochka'*3)
    test(zadaniya2.spam((1)), 'bulochka')
    test(zadaniya2.spam((8)), 'bulochka'*8)
